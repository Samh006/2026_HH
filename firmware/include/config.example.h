// config.example.h -- THE COMMITTED TEMPLATE. Copy it, then fill it in.
//
...
// ── Wi-Fi ───────────────────────────────────────────────────────────────
// Two networks, tried in order. Three things disqualify a network, and none
// of them are fixable in firmware:
//   * 5 GHz only          -- the S3 radio is 2.4 GHz. See WIFI_SCAN_AT_BOOT.
//   * WPA2-Enterprise     -- we do pre-shared keys only. This is Curtin.
//   * client isolation    -- common on guest Wi-Fi; the board associates
//                            fine and then cannot reach the server at all.
// The server has to be on the SAME network as the board, whichever it is.
#define WIFI_SSID_1     "your-2.4GHz-ssid"
#define WIFI_PASS_1     "your-password"
#define WIFI_SSID_2     "fallback-hotspot"
#define WIFI_PASS_2     "fallback-password"
#define WIFI_TIMEOUT_MS 15000

// Print every network the board's own radio can see, at boot, before trying
// to join anything. The S3 is 2.4 GHz ONLY, so an SSID that is 5 GHz-only --
// or WPA2-Enterprise, which our code cannot do either -- is invisible here
// and unjoinable no matter what the laptop next to it manages. Costs a couple
// of seconds of boot; set to 0 once the network is settled.
#define WIFI_SCAN_AT_BOOT 1
...
#define SERVER_BASE_URL      "http://192.168.1.100:5148"
