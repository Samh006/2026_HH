...
def main():
    ...
    ap.add_argument("--voice-sample", default="tools/cloud_voice_sample.wav",
                    help="a clip of the chosen OpenRouter TTS voice, to clone")
    ...
    voice = model.get_state_for_audio_prompt(args.voice_sample)
    ...
    for key in todo:
        ...
        if os.path.exists(path) and not args.force:
            skipped.append(key)
            continue
        audio = np.asarray(model.generate_audio(voice, PHRASES[key]),
                           dtype=np.float32)
        pcm = write_wav_f32_as_int16(path, audio, TARGET_SR)
        secs = len(pcm) / TARGET_SR
        flag = "  ! over 2s, shorten it" if secs > 2.0 else ""
